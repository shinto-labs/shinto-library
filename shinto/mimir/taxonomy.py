...
def get_taxonomy_by_id(connection: Connection, action_by: uuid.UUID, taxonomy_id: uuid.UUID, timestamp: Optional[Union[datetime, str]] = None) -> dict:
...
def get_taxonomy_by_name(connection: Connection, action_by: uuid.UUID, taxonomy_name: str, timestamp: Optional[Union[datetime, str]] = None) -> dict:
...
def get_taxonomy_history(connection: Connection, action_by: uuid.UUID, taxonomy_id: uuid.UUID) -> list[dict]:
...
def get_taxonomy_list(connection: Connection, action_by: uuid.UUID, timestamp: Optional[Union[datetime, str]] = None) -> list[dict]:
...
def create_taxonomy(connection: Connection, action_by: uuid.UUID, name: str, data: Optional[dict]) -> dict:
    """Create a taxonomy."""
    result = connection.execute_query(
...
def update_taxonomy(connection: Connection, action_by: uuid.UUID, taxonomy_id: uuid.UUID, name: Optional[str] = None, data: Optional[dict] = None ) -> dict:
...
def delete_taxonomy(connection: Connection, action_by: uuid.UUID, taxonomy_id: uuid.UUID) -> dict:
    """Delete a taxonomy. Accepts timestamp as datetime, ISO 8601 string, or None."""
    timestamp = normalize_timestamp(timestamp)
    result = connection.execute_query(
        "SELECT to_json(data.delete_taxonomy(%s::uuid, %s::uuid))",
        (action_by, taxonomy_id),
    )
    return result[0][0] if result else {}
